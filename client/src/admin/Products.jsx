  ...
  const handleSaveEdit = (editedProduct) => {
    ...
    axios
      ...
      .then((response) => {
        ...
        alert(`Successfully saved product: ${editedProduct.title}`);
        // Close the edit form
        setEditingProduct(null);
  ...
  const handleAddProduct = (newProduct) => {
    axios
      ...
      .then((response) => {
        ...
        alert(`Added product: ${newProduct.title}`);
    ...
    <div>
      <div className="w-full max-w-3xl mx-auto p-4">
        ...
        <div className="flex items-center space-x-2">
          ...
          <button
            className="px-4 py-2 bg-[#2c726b] text-sm text-white rounded-lg"
          ...
          <tbody>
            {displayedProducts.map((product) => (
              <tr key={product.id} className="border-b border-gray-300">
                <td className="p-2 text-sm">{product.title}</td>
                <td className="p-2 text-sm">${product.price}</td>
                <td className="p-2 text-sm">{product.category}</td>
                <td className="p-2 text-sm">{product.rating.rate}</td>
                <td className="p-2 text-sm">{product.rating.count}</td>
                <td className="p-2 text-sm">
                  <div className="flex">
                    <button
                      className="px-4 py-2 bg-[#DBC9B1] text-[#2c726b] rounded-lg"
                      onClick={() => handleEditProduct(product)}
                    >
                      Edit
                    </button>
                    <button
                      className="ml-2 px-4 py-2 bg-[#2c726b] text-white rounded-lg"
